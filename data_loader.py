...
class TextDS:
    ...
    def set_windowsize(self, window_size):
        self.window_size = window_size
    ...
    def batch_method(self, batch):
        ...
        for sentence in sentences:
            ...
            if sentence_len > 2*w+1:
                for i in range(w+1, sentence_len-(w+1)):
                    l1 = sentence[i - w:i] + sentence[i + 1:i + w + 1]
                    x.append(l1)
                    y.append(sentence[i])
        return x, y
...
class LabelledTextDS(TextDS):
    ...
    def get_vector_representation(self, maxdf = 0.4, maxfeatures=10000):
        """
        ...
        vectorizer = CountVectorizer(lowercase=False,
                                     ...
                                     stop_words=None,
                                     max_df=maxdf,
                                     max_features=maxfeatures,  ## This only keeps the 300 most frequent words as features
                                     ngram_range=(1, 1),  ## This uses only unigram counts
                                     binary=False)  ## This sets the features to be frequency counts
